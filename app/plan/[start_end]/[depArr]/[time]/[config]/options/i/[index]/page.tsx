...
import { ScheduleW700 } from "@material-symbols-svg/react/icons/schedule";
...
export default function Content() {
    ...
    return (
        <>
            <div className="rounded-xl border-3 p-2">
                <IconItem icon={{children: <ScheduleW700></ScheduleW700>}}>
                    <span className="text-lg font-medium">{node.duration >= 3600 && `${Math.floor(node.duration / 3600)}h `}{Math.floor(node.duration / 60 % 60)}min</span>
                    <span className="text-md font-medium text-darkgray">{format(new TZDate(node.start), "HH:mm")} - {format(new TZDate(node.end), "HH:mm")}</span>
                </IconItem>
                <div className="text-lg">{node.source == "DIGITRANSIT" ? "Generated by Digitransit" : "Generated by Hekinav"}</div>
            </div>
                        ...
                        return [<div key={i} className="w-full h-full flex-col flex">
                            {(l?.transitLeg || i == 0) && <div className="flex flex-row w-full" key={i + "a"}>
                                ...
                                {l?.transitLeg ? <Link className="decoration-none" href={`/stop/${l.from.stop?.gtfsId}/${isHsl ? "?hsl" : ""}`}><div className="p-2 flex flex-col font-medium my-2">
                                    ...
                                    <div><span className="text-lg font-medium">{l?.from.name == "START" ? origin.label : l.from.name}</span></div>
                                </div>}
                            </div>}
                            ...
                            {(l?.transitLeg || i == a.length - 1 || l?.to.viaType) && !next?.transitLeg && <div className="flex flex-row w-full" key={i + "c"}>
                                ...
                                {l?.transitLeg ? <Link className="decoration-none" href={`/stop/${l.to.stop?.gtfsId}/${isHsl ? "?hsl" : ""}`}><div className="p-2 flex flex-col font-medium my-2">
                                    ...
                                    <div><span className="text-lg font-medium">{l?.to.name == "END" ? destination.label : l.to.name}</span></div>
                                </div>}
                            </div>}
                        </div>
                        ]
