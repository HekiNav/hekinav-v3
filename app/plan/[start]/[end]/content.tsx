...
export default function Content({ data,
    destination,
    origin,
    isHsl }: ContentProps & PlanQueryQueryVariables) {
        ...
        <>
            ...
            <div className="flex flex-col gap-2">
                {data.edges?.map((e, i) => {
                    ...
                    return (
                        <div key={i} className="border-3 focus:border-green rounded-xl py-1 px-2 flex flex-col gap-1">
                            ...
                            <div className="w-full flex justify-between items-end">
                                {firstTransitLeg
                                    ...
                                    {(e?.node.waitingTime as number) > 600 && <IconItem className="gap-1! text-md" icon={{ children: <Hourglass width={16} height={16}></Hourglass> }}>{Math.round((e?.node.waitingTime as number) / 60)} min</IconItem>}
                                    <IconItem className="gap-0! text-md" icon={{ children: <DirectionsWalk width={16} height={16}></DirectionsWalk> }}>{walkDistance >= 1100 ? Math.round(walkDistance / 100) / 10 + " km" : Math.round(walkDistance) + " m"}</IconItem>
                                </span>
                            </div>
                        </div>
                    )
