...
import { ArrowLeft, ArrowRight } from "@material-symbols-svg/react"
...
export default function DotNavigationThingy({ onSet, amount, selected }: DotNavigationThingyProps) {
    ...
    return <div className="flex flex-row w-full items-center justify-center">
        <Icon onClick={() => onClick(selected - 1)} className={`py-1 pl-1 ${selected == 0 ? "text-stone-500" : "text-stone-800 cursor-pointer"}`}><ArrowLeft /></Icon>
        <div className="flex flex-row flex-nowrap gap-1 items-center justify-center">{...circles}</div>
        <Icon onClick={() => onClick(selected + 1)} className={`py-1 pl-1 ${selected == amount - 1 ? "text-stone-500" : "text-stone-800 cursor-pointer"}`}><ArrowRight/></Icon>
    </div>
}