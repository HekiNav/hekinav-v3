    ...
    return (
        <>
            <span className="flex justify-start items-center gap-2 mb-2 ml-14 mt-1">
                ...
                <Icon onMouseDownCapture={(e) => { e.stopPropagation(); e.preventDefault(); setConfig(config.favourites.routes.every(f => data.route?.gtfsId != f) ? [...config.favourites.routes, data.route?.gtfsId] : config.favourites.routes.filter(f => data.route?.gtfsId != f), ["favourites", "routes"]) }} className="ml-auto cursor-pointer">{config.favourites.routes.every(f => data.route?.gtfsId != f) ? <StarW700 className="hover:text-yellow"></StarW700> : <StarFillW700 className="text-yellow"></StarFillW700>}</Icon>
            </span>
